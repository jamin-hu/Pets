...
public final class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        Integer gender = values.getAsInteger(PetEntry.COLUMN_PET_GENDER); //why is this variable type Integer instead of int
        if (gender == null || !PetEntry.isValidGender(gender)) {
            throw new IllegalArgumentException("Pet requires a valid gender");
        }

        Integer weight = values.getAsInteger(PetEntry.COLUMN_PET_WEIGHT);
        if (weight != null && weight < 0) { //hmmmmmmmm compare with if (weight == null || weight < 0 )...
